...
const PRESET_ARTICLES = {
  ...
  'ai-security-c2': `AI安全：开源与闭源 C2 框架全景（2026-02）\n\n核心问题：C2 框架既是红队基础设施，也是防守侧需要重点监控的工具链。\n开源代表（含 GitHub）：Sliver、Mythic、Havoc、Covenant、Merlin、Empire、PoshC2、CALDERA、Metasploit、Koadic。\n闭源代表：Cobalt Strike、Brute Ratel C4、Nighthawk、Outflank C2。\n\n优势特征：\n- Sliver / Mythic / Havoc：跨平台、多用户协作、生态扩展强。\n- Covenant / PoshC2：.NET / PowerShell 生态与协作能力。\n- Merlin / Empire / CALDERA：多协议、对抗模拟与自动化编排。\n- Cobalt Strike / Brute Ratel / Nighthawk / Outflank：企业支持、稳定性与合规流程更完整。\n\n对比维度：架构协作、生态与插件、可审计性与合规、成本与支持、防守可观测。\n趋势判断：开源更灵活、扩展快；闭源在稳定性、合规与企业支持上占优。\n\n安全视角：防守要关注可观测性、策略基线、演练治理与内部使用规范。`,
  'opencode-architecture': `OpenCode 项目拆解：Agent Code 架构设计全景（2026-02-28）\n\n文章目标：理解 OpenCode 的技术栈、目录结构、Agent 执行闭环与前后端边界。\n\n核心技术栈：\n- 前端：SolidJS + Vite（Web）、Tauri（Desktop）、CLI（Bun）\n- 后端：Bun Runtime + Hono API + Session Loop\n- 模型层：Vercel AI SDK + 多 Provider 适配\n- 数据层：SQLite（Drizzle）+ Session/Message/Snapshot\n\n关键目录：\n- packages/opencode/src/index.ts：CLI 入口与命令注册\n- packages/opencode/src/server/server.ts：服务与路由挂载\n- packages/opencode/src/session/prompt.ts：Agent 主循环\n- packages/opencode/src/session/processor.ts：流式事件处理\n- packages/opencode/src/tool/registry.ts：工具注册与过滤\n- packages/opencode/src/permission/next.ts：权限决策引擎\n\nAgent 运行链路：输入归一 -> 上下文装配 -> 模型流式推理 -> 工具调用与权限校验 -> 状态回写 -> UI 实时更新。\n\n设计结论：OpenCode 把“推理、工具、安全、状态”统一成一个可观测、可扩展、可治理的 Agent Runtime。`,
  'china-cybersecurity-industry-2026': `中国网络安全行业全景：领域、岗位与技术能力地图（2026）\n\n行业判断：网络安全正在从“单点采购”进入“持续运营 + 合规治理 + 工程化落地”阶段。\n政策节点：网络数据安全管理条例已于 2025-01-01 施行；网络安全法修订决定自 2026-01-01 起施行。\n规模参考：2023 年中国网络安全产业规模约 823 亿元。\n人才趋势：公开报告显示人才缺口仍在 300 万级，企业更看重实操能力与项目经验。\n\n十大赛道：SOC、攻防渗透、数据安全、云安全与 DevSecOps、应用与供应链安全、身份安全、工控/车联网、GRC 合规、AI 安全、安全产品研发。\n\n关键岗位：SOC 分析师、应急响应、渗透测试、红队、云安全工程师、数据安全工程师、隐私合规、等保测评、AI 安全工程师、安全架构师。\n\n求职建议：先选赛道，再补技术栈；优先准备“漏洞复现 + 检测规则 + 合规整改”三类可证明成果。`,
};
          ...
          <section className="mt-10 grid items-center gap-10 lg:grid-cols-[1.1fr_0.9fr]">
            <div>
              ...
              <h1 className="mt-4 text-3xl font-semibold leading-tight text-ink sm:text-4xl">
                用提示词驱动 AI，
                <span className="theme-gradient-text">生成高质量微信图文卡片</span>
              </h1>
              ...
              <div className="mt-6 grid gap-3 sm:grid-cols-3">
                {[
                  ...
                  <div key={item.title} className="glass-card rounded-2xl p-3 text-xs text-muted">
          ...
          <div className="mt-6">
            ...
            <div className="mt-12">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <span className="soft-button soft-button-secondary px-3 py-1 text-[11px] text-muted">
